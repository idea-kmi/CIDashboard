...
class MCAPI {
    ...
    */
    ...
    function callServer($method, $params) {
		...
		/** MB: for handling proxy **/
        $postUrl = $proxy ? "http://$host/".$this->apiUrl["path"] : $this->apiUrl["path"];
		$payload = "POST " . $postUrl . "?" . $this->apiUrl["query"] . "&method=" . $method . " HTTP/1.0\r\n";
		...
		/** MB: for handling proxy **/
        if ($proxy) {
		    $sock = fsockopen($proxy, $proxyPort, $errno, $errstr, 30);
		} else if ($this->secure){
